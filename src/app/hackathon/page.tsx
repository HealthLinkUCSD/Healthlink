...
export default function HackathonPage() {
  return <main className="min-h-screen bg-gradient-to-b from-[#071225] via-[#0a1b35] to-[#102647] text-white">
    ...
    <div className="mx-auto max-w-6xl space-y-20 px-6 pb-20">
      ...
      <section className="space-y-6"><div><p className="text-xs uppercase tracking-[0.2em] text-blue-300">Last year at HealthLink</p><h2 className="mt-3 text-4xl font-extrabold">A look back. A new beginning.</h2></div><div className="grid gap-6 sm:grid-cols-3">{["Photo highlights", "Prizes & winning builds", "Last year's sponsors"].map(title => <article key={title} className="flex min-h-52 flex-col justify-end rounded-3xl border border-blue-500/30 bg-gradient-to-br from-blue-500/10 to-white/5 p-6"><h3 className="text-xl font-bold">{title}</h3><p className="mt-3 text-neutral-400">Coming soon</p></article>)}</div></section>
      <section id="signup" className="scroll-mt-28 rounded-3xl border border-blue-400/40 bg-blue-500/10 p-8 text-center sm:p-12"><p className="text-xs uppercase tracking-[0.2em] text-blue-300">Applications</p><h2 className="mt-3 text-4xl font-extrabold">Make time to build.</h2><p className="mt-4 text-lg text-neutral-200">Signup deadline: September 30, 2026.</p><p className="mt-5 rounded-xl border border-white/10 bg-white/5 p-4 text-blue-200">Google signup form coming soon.</p><p className="mt-4 text-sm text-neutral-300">Questions? <a className="underline hover:text-blue-200" href="mailto:healthlink@ucsd.edu">healthlink@ucsd.edu</a></p></section>
    </div>
  </main>;
}
